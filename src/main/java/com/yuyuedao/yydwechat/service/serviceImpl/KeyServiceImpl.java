...
import com.yuyuedao.yydwechat.mapper.generator.WPKeyMapper;
...
public class KeyServiceImpl extends PublicMethod implements KeyService  {
    ...
    @Override
    public int edit(WPKey key) {

        Integer count=keyMapper.checkName(key.getKeyword(),null,"update");
        if(count>0){
            return -1;
        }else{
            return wpKeyMapper.updateByPrimaryKeySelective(key);
        }
