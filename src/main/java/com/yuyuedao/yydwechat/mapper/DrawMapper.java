...
public interface DrawMapper {
    ...
    @Select("select a.openid,head_url,convert(nvarchar(10),a.createtime,120),nickname from activity_participant a,w_p_fans b where a.openid=b.openid  and a.invate_openid=#{openid} and stype='draw' and a.activityId=#{activityId} ")
    List<DrawUser> getDrawDetail(@Param("activityId")Integer activityId,@Param("openid") String openid);


    @Select("select a.openid,head_url,convert(nvarchar(10),a.createtime,120),nickname from activity_participant a,w_p_fans b where a.openid=b.openid  and a.invate_openid=#{openid} and stype='poster' and a.activityId=#{activityId} ")
